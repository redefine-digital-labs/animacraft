...
test('later v8 callables may upgrade while one stable TypeOrigin remains pinned', () => {
  ...
  for (const field of MAKER_V8_RETIRED_SPLIT_FIELDS) {
    const split = inspectMakerV8Runtime(runtime({ [field]: CORE }));
    assert.equal(split.issues.some((entry) => (
      entry.code === 'MAKER_V8_SPLIT_RUNTIME_FIELD_RETIRED' && entry.field === field
    )), true, field);
  }
});
