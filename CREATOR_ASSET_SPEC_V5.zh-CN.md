...
Item 自己不保存 PNG 和坐标。它负责名称、缩略图、顺序、默认 Style 和组合条件；发布可用性由统一发布检查判定，不要求创作者逐个维护隐藏的发布状态字段。
...
- `scale = 1` 表示 PNG 原始尺寸。
- `rotation = 0` 表示不旋转。
- 旋转中心默认为 PNG 自身中心；旋转不会再围绕左上角把素材甩离角色。
...
当 PNG 尺寸与画布不一致时，编辑器按整张 PNG 的尺寸等比适配，并把整张 PNG 自动放到画布中央。透明像素边界只用于缩略图裁切和对齐诊断，不参与初始坐标计算；源 PNG 不被裁切或改写。
...
- 同一 Part 内仅改变肤色或配色的轮廓变体，alpha mask IoU 不低于 `0.95`；
- 声明为同锚点的变体，可见内容中心漂移不超过画布宽高的 `1%`；
- 检测空 PNG 和非预期的四边触边裁切；
- 使用统一 Renderer 生成默认 Recipe golden image；
- 自动生成关键 Part 的 pairwise 组合 contact sheet，由真人逐格验收；
- Maker 封面若代表默认造型，必须由同一个 Renderer 和默认 Recipe 生成。
...
当前仓库中的 Astral Courier release 与 Angie release 为同一批素材，它必须标记为
`doNotPublish / doNotUseAsVisualGold`，不得以 Mainnet 可铸造模板发布。Hanamori
可以作为较稳定的内部正向压力样例，但在头发、服装、表情逐项人工清稿并完成组合验收前，
同样不能作为社区创作者的视觉金标准。
