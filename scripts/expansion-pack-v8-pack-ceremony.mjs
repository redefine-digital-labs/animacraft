...
export async function main(argv = process.argv.slice(2), adapters = createDefaultCeremonyAdapters()) {
  ...
  if (args.command === 'init') {
    ...
    output = await initializeCeremony({ repoRoot: resolve(new URL('..', import.meta.url).pathname),
      readinessPath: resolve(args.readiness), statePath, nonce: args.nonce });
  } else if (args.command === 'status') output = await ceremonyStatus(statePath);
  else if (args.command === 'lock') output = await lockCurrentAction(statePath, args, adapters);
  else if (args.command === 'execute') output = await executeCurrentAction(statePath, args, adapters);
  else if (args.command === 'recover') output = await recoverSubmittedAction(statePath, args, adapters);
  else output = await ceremonyReceipt(statePath);
