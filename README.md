...
Creator Studio keeps `Parts & Items` as the persistent artwork workspace. Layer Tracks, Smart Color, Rules, Expansion Packs, and Preflight open as bounded tool dialogs over that workspace and close without losing the selected Part or layer. Draft text is flushed before Save, Undo, tool switching, and publication review. The core Maker workspace follows the MyPage language setting for English, Chinese, Japanese, Korean, and Vietnamese; untranslated protocol diagnostics still fall back to English rather than hiding an error.
