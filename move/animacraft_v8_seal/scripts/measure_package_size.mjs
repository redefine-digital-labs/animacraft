...
const buildDir = path.join(packageDir, 'build', 'animacraft_v8_seal');
const bytecodeDir = path.join(buildDir, 'bytecode_modules');
const disassemblyDir = path.join(buildDir, 'disassembly');
const targetBytes = 75_000;
const hardMaxBytes = 102_400;
const requiredHardMaxHeadroomBytes = 10_000;

if (!fs.existsSync(bytecodeDir) || !fs.existsSync(disassemblyDir)) {
  throw new Error('Run `sui move build --force --disassemble --warnings-are-errors` first.');
}
...
// Exact Sui MovePackage::size(): SequenceNumber + module map + TypeOrigins + linkage.
const packageObjectBytes = 8 + moduleMapBytes + typeOriginBytes + linkageBytes;
const hardMaxHeadroomBytes = hardMaxBytes - packageObjectBytes;
const result = {
  ...
  hardMaxBytes,
  requiredHardMaxHeadroomBytes,
  hardMaxHeadroomBytes,
...
if (packageObjectBytes > targetBytes || hardMaxHeadroomBytes < requiredHardMaxHeadroomBytes) {
  process.exitCode = 1;
}
