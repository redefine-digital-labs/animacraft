...
const targetBytes = 45_000;
const hardMaxBytes = 102_400;

if (!fs.existsSync(bytecodeDir) || !fs.existsSync(disassemblyDir)) {
  throw new Error(
    'Run `sui move build --disassemble --warnings-are-errors` in move/animacraft_v8_core first.',
  );
}
...
const typeOrigins = modules.flatMap((module) =>
  module.datatypeNames.map((datatypeName) => ({
...
// Exact Sui MovePackage::size(): SequenceNumber, module map, TypeOrigins,
// and linkage entries. Source, tests, docs, and build metadata are excluded.
const packageObjectBytes = 8 + moduleMapBytes + typeOriginBytes + linkageBytes;
const result = {
  formula: 'MovePackage::size',
  moduleBytecodeBytes: modules.reduce(
    (total, module) => total + module.bytecodeBytes,
    0,
  ),
  moduleMapBytes,
