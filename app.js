...
export function createSuiV8BrowserAdapters({ client, wallet, dataSource, transactions, persistence }) {
  ...
  return assertFreshV8Adapters({
    rpc: {
      ...
      queryTransaction: ({ digest, identity }) => dataSource.queryTransaction({ client, digest, identity }),
...
export function assertFinalizedMarketReadbackV8(value, request, marketClient, marketModule) {
  ...
  if (value.schemaVersion !== WEB_V8_READBACK_SCHEMA || value.source !== 'FINALIZED_RPC'
    ...
    throw appError('WEB_V8_FINALIZED_READBACK_MISMATCH', 'Finalized readback does not bind the exact digest, checkpoint, and identity.', 'READBACK');
  }
  if (value.event?.id?.txDigest !== request.digest) {
  ...
  if (action.kind === 'LIST' && event.fields.rootId !== request.identity.root.id) {
    throw appError('WEB_V8_FINALIZED_EVENT_MISMATCH', 'Opened listing event binds another Root.', 'READBACK');
  }
  ...
  if (action.kind === 'RECOVER' && event.fields.recovered !== true) {
    ...
    async compareAndSwap(scopeKey, expectedRevision, nextRecord, options = {}) {
      ...
      if (nextRecord) {
        ...
        if (!options.completionReceipt) {
          transaction.abort();
          await done.catch(() => {});
          throw appError('MAKER_V8_RECOVERY_RECEIPT_INVALID', 'Verified cleanup requires its exact receipt.', 'STORAGE');
        }
        transaction.objectStore('receipts').put(options.completionReceipt, options.completionReceipt.identityKey);
...
export function createFreshV8Controller({
  ...
  const recovery = recoveryModule.createMakerV8RecoveryController({
    ...
    query: ({ digest, identity }) => adapters.rpc.queryTransaction({ digest, identity }),
  ...
  async function loadSelectedAction() {
    ...
    if (pendingAtRoot?.identity) {
      ...
      currentIdentity = state.recoveryRecord.identity;
    } else {
      currentIdentity = liveIdentity;
      state.recoveryRecord = await recovery.load(liveIdentity);
    }
    state.status = state.recoveryRecord?.state || 'QUOTING';
  }
